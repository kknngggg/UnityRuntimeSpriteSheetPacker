...
{
    public sealed class StandaloneWindowsFileBrowser : IFileBrowser
    {
        private const string LAST_DIR_KEY = "kknngggg.Unity.Sprites.Demos.SpriteSheets.IO.StandaloneWindowsFileBrowser.LAST_DIR_KEY";
        ...
        public void SaveFile(string fileName, ReadOnlySpan<byte> data, string fileExtension = "")
        {
            ...
            {
                Debug.LogError($"[StandaloneWindowsFileBrowser] Failed to save '{fullSavePath}': {exception.Message}");
            }
        }

        private static string OpenFilePanel(string[] extensions)
        {
            return ShowDialog(save: false,
                              "Select File",
                              LastDirectory,
        ...
        private static bool TryGetFullSavePath(string fileName, string fileExtension, out string fullSavePath)
        {
            ...
            fullSavePath = ShowDialog(save: true,
                                      "Save File",
                                      LastDirectory,
            ...
            string defaultExtension)
        {
            ...
            try
            {
                ...
                if (ok)
                {
                    return Marshal.PtrToStringUni(filePtr) ?? string.Empty;
                }

                int error = CommDlgExtendedError();
                if (error != 0)
                {
                    Debug.LogError($"[StandaloneWindowsFileBrowser] Native file dialog failed. CommDlgExtendedError=0x{error:X8}");
                }
